...
export default function usePhoneCode(onContinue: () => void) {
    return (
        <>
            {phoneInput()}
            <div className="flex flex-row justify-center">
                <input className="mt-4 block w-3/4 px-3 py-2 bg-white border border-slate-300 rounded-md text-sm shadow-sm placeholder-slate-400
                    focus:outline-none focus:border- focus:ring-1 focus:ring-railway_blue
                    disabled:bg-slate-50 disabled:text-slate-500 disabled:border-slate-200 disabled:shadow-none
                    invalid:border-pink-500 invalid:text-pink-600
                    focus:invalid:border-pink-500 focus:invalid:ring-pink-500" type="password" placeholder="验证码" />
                <button className="border text-xs w-1/4 ml-3 mt-4  border-railway_blue rounded-md text-railway_blue h-9">发送</button>
            </div>
